...
import { PackCellData, CellData, PackData } from "./MergeDataInterface"

const { ccclass, property } = cc._decorator;
@ccclass
export default class MergePackDialog extends MyComponent {
    @property(cc.Label)
    ...
    private init () {
        ...
        for (let i = 0; i < pack_data.own; ++i) {
            ...
            if (cell_data) {
                ...
                event_handler.component = "MergePackDialog";
    ...
    private setItem (node: cc.Node, cell_data: PackCellData) {
        if (cell_data) {
            this._resource_manager.getSpriteFrame(`merge/ele/${cell_data.icon}`).then((sprite_frame) => {
                if (cc.isValid(node)) {
