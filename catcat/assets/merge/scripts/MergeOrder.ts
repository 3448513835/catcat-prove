...
import MergeOrderItem from "./MergeOrderItem"
import { MergeData, MapData } from "./MergeData"
import { ShopData, ShopItem } from "./MergeDataInterface"
...
export default class MergeOrder extends MyComponent {
    ...
    private clickOpenOrderDialog () {
        this._dialog_manager.openDialog(this._dialog_name.MergeOrderDialog, {
    ...
    private onRefrushOrder (data) {
        ...
        if (!data.list) { // 刷新finish按钮
            ...
            this.my_scrollview.node.width = 897;
            if (this.order_list.length < 3) {
                this.my_scrollview.node.width = this.order_list.length*ORDER_ITEM_WIDTH;
            }
            cc.find("view", this.my_scrollview.node).width = this.my_scrollview.node.width;
            let tmp_list = [];
    ...
    private setCellCountList () {
        this.cell_count_list = {};
        let cell_data_list = this.merge_data.cell_data_list;
        for (let item of cell_data_list) {
    ...
    private initShopData () {
        const LOCAL_KEY = "MERGE_SHOP_DATA"; // 本地KEY
        ...
        this.scheduleOnce(() => { this.initShopData(); }, rest_tm1+1);
    }
