...
import { MergeData, MapData } from "./MergeData"
import { SpecialElementTypes } from "./MergeDataInterface"


const { ccclass, property } = cc._decorator;
@ccclass
export default class MergeElementDialog extends MyComponent {
    @property(cc.Label)
    ...
    onLoad () {
        ...
        let group = this._json_manager.getJsonData(this._json_name.ELE, this.element_id).group;
    ...
    private setElementLayout () {
        for (let i = 0; i < this.group_element_list.length; ++i) {
            ...
            if (index != -1 || ele_json.id == this.element_id) {
                if (ele_json.is_use != 0) {
                    cc.find("Use", node).active = true;
                }
                this._resource_manager.getSpriteFrame(`merge/ele/${ele_json.icon}`).then((sprite_frame) => {
                ...
                if (!button) {
                    ...
                    handler.component = "MergeElementDialog";
    ...
    private setFromLayout () {
        let json_data = this._json_manager.getJsonData(this._json_name.ELE, this.element_id);
        ...
        for (let i = 0; i < output_way.length; ++i) {
            ...
            node.active = true;
            let out_json = this._json_manager.getJsonData(this._json_name.ELE, output_way[i]);
            let sprite = cc.find("Icon", node).getComponent(cc.Sprite);
            if (true || this.map_data.element_record.indexOf(out_json.id) != -1) {
                ...
                this._resource_manager.getSpriteFrame(`merge/ele/${out_json.icon}`).then((sprite_frame) => {
                    if (cc.isValid(sprite)) {
                        sprite.spriteFrame = sprite_frame;
                        sprite.node.scale = 0.5;
                    }
    ...
    private clickElement (event: cc.Event.EventTouch) {
        ...
        if (ele_json.is_use && ele_json.use_value && SpecialElementTypes.indexOf(ele_json.type) == -1) {
            ...
            let from_ele_json = this._json_manager.getJsonData(this._json_name.ELE, output_way[index]);
    ...
    private clickFromItem (event: cc.Event.EventTouch, param) {
        ...
        let ele_json = this._json_manager.getJsonData(this._json_name.ELE, this.element_id);
        let output_way = ele_json.output_way.split(",");
        var _dialog_manager = this._dialog_manager;
        _dialog_manager.closeDialog(this._dialog_name.MergeElementDialog);
        var ele_id = output_way[index];
        _dialog_manager.openDialog(this._dialog_name.MergeElementDialog, {
            element_id: ele_id,
        });

        // let from_ele_json = this._json_manager.getJsonData(this._json_name.ELE, output_way[index]);
        // let sprite = cc.find("Item/Icon", this.from_tip_node).getComponent(cc.Sprite);
        // this._resource_manager.getSpriteFrame(`merge/ele/${from_ele_json.icon}`).then((sprite_frame) => {
        //     if (cc.isValid(sprite)) {
        //         sprite.spriteFrame = sprite_frame;
        //     }
        // });
        ...
        // // let drop_id = ele_json.output_way.split(",")[index];
        // let drop_id = this._json_manager.getJsonData(this._json_name.ELE, ele_json.output_way.split(",")[index]).use_value;
        // let drop_json = this._json_manager.getJsonData(this._json_name.DROP, drop_id);
        // let layout = cc.find("From/Layout", this.from_tip_node);
        // let reward_ele_list = drop_json.reward_ele.split(",");
        // for (let i = 0; i < reward_ele_list.length; ++i) {
