...
import { CellData, MapData } from "./MergeData"
...
@ccclass
export default class MergeUnlockTipDialog extends MyComponent {
    @property(cc.Node)
    ...
    private showLockPanel () {
        ...
        let json = this._json_manager.getJson(this._json_name.COM_LV);
        let need_exp = 0;
        for (let key in json) {
            let value = json[key];
    ...
    private showTiles () {
        for (let cell of this.cell_data_list) {
            ...
            this._resource_manager.getSpriteFrame(`merge/ele/${cell.icon}`).then((sprite_frame) => {
