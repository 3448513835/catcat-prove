...
export default class MergeBottom extends MyComponent {
    ...
    public setData (cell_data: CellData) {
        ...
        if (!this.cell_data || !this.cell_data.element) {
            ...
            let json_data = this._json_manager.getJsonData(this._json_name.ELE, this.cell_data.element);
            ...
            // if (this.cell_data.use && this.cell_data.use.count == 0) { // 加速
    ...
    private clickSpeed () {
        ...
        if (this.cell_data) {
            ...
            let json_data = this._json_manager.getJsonData(this._json_name.ELE, this.tmp_bubble_data.id);
