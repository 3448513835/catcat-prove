...
import { ShopItem } from "./MergeDataInterface"
...
export default class MergeShopItem extends MyComponent {
    ...
    public setData (data: ShopItem, video_spriteframes: cc.SpriteFrame[]) {
        ...
        if (this.data.reward_type == 1) { // 元素
            let item_json = this._json_manager.getJsonData(this._json_name.ELE, this.data.reward);
            icon_url = "merge/ele/"+item_json.icon;
