...
import { MapData } from "./MergeDataInterface2d"
import MergeOrderDetailItem2d from "./MergeOrderDetailItem2d"

const { ccclass, property } = cc._decorator;
@ccclass
export default class MergeOrderDialog2d extends MyComponent {
    @property(cc.Label)
    ...
    private updateScrollviewItem(node: cc.Node, index: number) {
        node.getComponent(MergeOrderDetailItem2d).setData(this.order_list[index].id, this.cell_count_list, this.map_data);
    }
}
