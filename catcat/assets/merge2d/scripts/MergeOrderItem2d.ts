...
export default class MergeOrderItem extends MyComponent {
    ...
    public setData (order_data: OrderData, cell_count_list: any, map_data, shop_list) {
        ...
        let json_data = this._json_manager.getJsonData(this._json_name.ORDER_2D, this.order_data.id);
        ...
        for (let i = 0; i < order_info_list.length; ++i) {
            ...
            let icon_url = this._json_manager.getJsonData(this._json_name.ELE_2D, ele_id).icon;
            this._resource_manager.getSpriteFrame(`merge2d/ele/${icon_url}`).then((sprite_frame) => {
            ...
            let has_count = 0;
            if (this.cell_count_list[ele_id]) {
                has_count = this.cell_count_list[ele_id].length;
            }
    ...
    private clickFinish () {
        ...
        let order_json = this._json_manager.getJsonData(this._json_name.ORDER_2D, this.order_data.id);
    ...
    private click (event: cc.Event.EventTouch, param) {
        let guide_id = GuideManager.getGuideId();
        if (guide_id == 7) {
            this.clickFinish();
            return;
        ...
        let sale_node = cc.find("Sale", event.currentTarget.parent);
        if (cc.isValid(sale_node) && sale_node.active) {
            this._dialog_manager.openDialog(this._dialog_name.MergeShopDialog2d, {
                jump_element: id,
            });
        }
        else {
