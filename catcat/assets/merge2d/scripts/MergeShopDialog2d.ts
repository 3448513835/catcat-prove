...
import { ShopData, ShopItem, MapData, SpecialElementTypes } from "./MergeDataInterface2d"
import MergeShopItem from "./MergeShopItem2d"
...
@ccclass
export default class MergeShopDialog2d extends MyComponent {
    @property(cc.Prefab)
    ...
    private jump_element = null;

    onLoad () {
        cc.sys.localStorage.removeItem("MERGE_SHOP_DATA_2D");
        ...
        this.jump_element = this.getDialogData().jump_element;
        this.initData();
    ...
    private refrushPanel () {
        ...
        let index = 0, y = this.content_node.parent.height/2, choose_node = null;
        this.schedule(() => {
            ...
            if (item) {
                ...
                if (item.reward == this.jump_element && index > 3) {
                    this.jump_element = null;
                    let layout = this.content_node.getComponent(cc.Layout);
                    y += Math.floor(index/3)*(layout.spacingY+node.height)+layout.paddingTop/2;
                    let content_height = Math.floor(this.shop_data.list.length/3)*(layout.spacingY+node.height)-layout.spacingY+layout.paddingTop+layout.paddingBottom;
                    if (y > content_height-this.content_node.parent.height/2) {
                        y = content_height-this.content_node.parent.height/2;
                    }
                    choose_node = node;
                }
                node.active = true;
                if (index == this.shop_data.list.length-1 && choose_node) {
                    let icon_node = cc.find("Icon", choose_node);
                    cc.tween(icon_node).to(0.5, { scale: 1.25 }).to(0.5, { scale: 1.0 })
                        .to(0.5, { scale: 1.25 }).to(0.5, { scale: 1.0 }).start();
                    this.content_node.y = y;
                }
            }
            ++index;
        }, 0, count-1);
    ...
    private onShopMsg (data) {
        ...
        if (shop_data.reward_type == 1) {
            let json_data = this._json_manager.getJsonData(this._json_name.ELE_2D, shop_data.reward);
            if (!json_data.output_way && SpecialElementTypes.indexOf(json_data.type) == -1) {
                ...
                this._dialog_manager.openDialog(this._dialog_name.MergeElementDialog2d, { element_id: shop_data.reward, });
            }
        }
