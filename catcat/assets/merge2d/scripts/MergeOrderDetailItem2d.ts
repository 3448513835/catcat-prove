...
@ccclass
export default class MergeOrderDetailItem2d extends MyComponent {
    @property([cc.Node])
    ...
    public setData (order_id: number, cell_count_list: any, map_data) {
        ...
        let json_data = this._json_manager.getJsonData(this._json_name.ORDER_2D, this.order_id);
        ...
        for (let i = 0; i < order_info_list.length; ++i) {
            ...
            let has_count = 0;
            if (cell_count_list[ele_id]) {
                has_count = cell_count_list[ele_id].length;
            }
